...
phenologyForecast <- function(siteName,URL,forecastLength=0,startDate=FALSE,endDate=FALSE,lat,long){
  ...
  GOES.data <- read.csv("dataFiles/GOES_NDVI_HarvardForest_2017-07-01_2018-06-30_noon.csv",header=FALSE)
...
ci <- phenologyForecast(siteName=as.character(siteData[1,1]),URL=as.character(siteData[1,4]),forecastLength = 30,lat=as.numeric(siteData[1,2]),long=as.numeric(siteData[1,3]))
